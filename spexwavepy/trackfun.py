...
class Tracking:
    ...
    def XST_withrefer(self, edge_x, edge_y, pad_x, pad_y, hw_xy, window=None, normalize=False, display=False, verbose=True):
        ...
        if scandim == 'xy':
            if edge_x[0] != edge_y[0] and edge_x[1] != edge_y[1] and edge_x[0] != edge_x[1] and edge_y[0] != edge_y[1]:
                print("For scandim == 'xy', the edges should be symmetrical, edge_x should be the same as edge_y, and also the elements within.")
                sys.exit(0)
        if scandim == 'xy':
            if pad_x[0] != pad_y[0] and pad_x[1] != pad_y[1] and pad_x[0] != pad_x[1] and pad_y[0] != pad_y[1]:
                print("For scandim == 'xy', the pad should be symmetrical, pad_x should be the same as pad_y, and also the elements within.")
                sys.exit(0)
    ...
    def XST_self(self, edge_x, edge_y, pad_x, pad_y, hw_xy, window=None, normalize=False, display=False, verbose=True):
        ...
        self.XST_withrefer(edge_x, edge_y, pad_x, pad_y, hw_xy, window, normalize, display, verbose)
        self.sloX = None
        self.sloY = None
        if scandim == 'x':
    ...
    def XST_self_multi(self, edge_x, edge_y, pad_x, pad_y, hw_xy, window, cpu_no, normalize=False, verbose=True):
        ...
        self.XST_withrefer_multi(edge_x, edge_y, pad_x, pad_y, hw_xy, window, cpu_no, normalize, verbose)
        self.sloX = None
        self.sloY = None
        if scandim == 'x':
            self.curvX = curv_pixel(self.delayX, self.scanstep, self.dist, self.pixsize, self.mempos)
